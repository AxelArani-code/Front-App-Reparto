      ...
      <div className="mx-4" ref={setNodeRef} style={style} {...(isDragEnabled ? { ...attributes, ...listeners } : {})}>
        <Card shadow="md" className="w-full">
          {isDragEnabled ? (
            ...
            <Link to="/view-orders-user" className="block"  state={{getFirstName: FirstName, getLastName:LastName, getTelephone:Telephone ,getClientEntityId:id, getDayEntityId:Day, getDay:Date, getAddress: Address, getDescription: Description}}>
              <CardHeader className="gap-4">
                <div className="w-2 h-10 bg-primary rounded" />
                <div>
                  <h2 className="text-lg font-bold text-primary">{FirstName} {LastName}</h2>
