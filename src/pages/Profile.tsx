        ...
        {menuItems.map((item) => (
            item.path ? (
                ...
                <Button  key={item.label} variant="ghost" className=" w-full  justify-between px-2 py-6 h-auto font-normal hover:bg-muted">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 flex items-center justify-center">
                      ...
                      { item.icon === "user" && (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" strokeLinecap="round" stroke-linejoin="round" className="lucide lucide-user"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>)
              ...
              <Button key={item.label} variant="ghost" className="w-full justify-between px-2 py-6 h-auto font-normal hover:bg-muted" onClick={() => onOut()}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 flex items-center justify-center">
                    <span className="text-muted-foreground">   {item.icon === "log-out" && (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" strokeLinecap="round" stroke-linejoin="round" className="lucide lucide-log-out"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" x2="9" y1="12" y2="12" /></svg>)}</span>
                  </div>
                  {item.label}
                </div>
                <ChevronRight className="h-5 w-5 text-muted-foreground" />
